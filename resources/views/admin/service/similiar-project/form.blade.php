    ...
    <div class="row my-6 py-6">
        <div class="col-md-12">
            <div class="card border-0">
                <div class="card-body">
                    <form action="{{ isset($similarProject) ? route('similar-project.update', $similarProject->id) : route('similar-project.store')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @isset($similarProject) @method('PUT') @endisset
                        ...
                        <div class="row">
                            <div class="col-lg-12 d-flex">
                                @foreach ($similarProjects as $data)
                                    <div class="image">
                                        <meta name="csrf-token" content="{{ csrf_token() }}">

                                        <img src="{{ asset($data->image) }}" alt="" id="gallery">
                                        <button type="button" data-id="{{ $data->id }}" data-url="{{ route('similar-project.destroy', $data->id)}}" class="deleteRecord btn text-danger">
...
@section('scripts')
    ...
    <script>
        $(document).ready(function(){
            $('#service_id').val("{{$similarProject->service_id ?? ''}}");
        });
    </script>
@endsection
